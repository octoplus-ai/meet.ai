...
function PlansView({ onBack, plan: currentPlan }) {
  ...
  const plans = [
    ...
    { key: "starter", name: "Starter", monthly: "$29", annual: "$23", note: "per seat / month",
    ...
    { key: "pro", name: "Pro", monthly: "$49", annual: "$39", note: "per seat / month", popular: true,
      features: ["Everything in Starter", "40 AI docs & 30 decks / month", "Priority processing", "Calendar browser extension"] },
    { key: "business", name: "Business", monthly: "$99", annual: "$79", note: "per seat / month · 3+ seats",
        ...
        <div className="flex-1 overflow-y-auto px-8 py-7">
          <div className="mx-auto max-w-2xl space-y-5">
            ...
            {sec === 3 && (<>
              ...
              <ToggleRow title="Show presenter during screen share" desc="On for every meeting: the active speaker's camera is composited straight into the recording (a small corner tile) whenever someone shares their screen - perfectly in sync, and it transitions as the speaker changes. Great for trainings. Turn it off for a clean full-screen share. Applies to new recordings." on={tg.pipDuringShare} onChange={(v) => set1("pipDuringShare", v)} />
              {tg.pipDuringShare && (
...
function MeetingVideo({ videoRef, src, coverAt, markers, turns, subtitles, meetingId, shareTok, coverDone, coverUrl, durationMin, dubs = {}, nameMap = {}, speakerTimeline = [], pipUrl = null, shareIntervals = [] }) {
  ...
  const [showPip, setShowPip] = useState(false); // training PiP: overlay the speaker's camera during a screen share - OFF by default (button turns it on); shows only when activated AND inside a share
  ...
  const generate = async () => {
    ...
    try {
      const sys = "You are a meeting-intelligence analyst. Read the transcript and return ONLY a JSON object (no markdown) with this shape:\n" +
        `{"summary": string - a THOROUGH multi-paragraph recap (3-5 full paragraphs, ~250-450 words, blank line between paragraphs, never one paragraph) in the meeting's language for someone who missed it: FIRST paragraph = context, purpose and the headline outcome; MIDDLE paragraphs = one per major theme with the WHY behind each and where it landed; FINAL paragraph = decisions and commitments (with owners) and what stays open; complement, never repeat, the topics/actionItems/nextSteps; stay specific and neutral with real names, numbers and decisions, no invented facts, "topics": string[] (max 5), "keyQuestions": string[] (max 4), "actionItems": [{"owner": string, "task": string, "due": string}] (max 6), "nextSteps": string[] (max 3), "participants": [{"name": string, "role": string, "talkPct": integer, "sentiment": "Positive"|"Neutral"|"Negative"}], "scores": {"overall": int, "engagement": int, "sentiment": int, "balance": int, "clarity": int} (0-100), "sentimentLabel": "Positive"|"Neutral"|"Negative", "sentimentTimeline": number[] (8 values -1..1)}.\n` +
