...
function docHTML(doc, meta) {
  ...
  const body = `<div class="title">${escD(doc.title || meta.title || "Meeting")}</div>`
    ...
    + (doc.tldr ? `<div class="tldr"><b>TL;DR</b><br>${escD(doc.tldr)}</div>` : "")
...
function DocModal({ doc, meta, onClose }) {
  const { full, title } = docHTML(doc, meta);
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 p-4" onClick={onClose}>
      <div className="flex max-h-[88vh] w-full max-w-3xl flex-col overflow-hidden rounded-2xl bg-white shadow-2xl" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between border-b border-slate-200 px-5 py-3">
          ...
          <div className="flex items-center gap-2">
            <button onClick={dlPDF} className="flex items-center gap-1.5 rounded-lg bg-violet-600 px-3 py-1.5 text-[13px] font-semibold text-white hover:bg-violet-500"><Download size={14} /> PDF</button>
            <button onClick={dlWord} className="flex items-center gap-1.5 rounded-lg border border-slate-200 px-3 py-1.5 text-[13px] font-semibold text-slate-600 hover:bg-slate-50"><Download size={14} /> Word</button>
            <button onClick={onClose} className="rounded-lg p-1.5 text-slate-400 hover:bg-slate-100"><X size={18} /></button>
          </div>
        </div>
        <iframe title="document" srcDoc={full} className="h-[70vh] w-full flex-1 bg-white" />
      </div>
    </div>
  );
}
...
function MeetingDetail({ meeting, onBack, onUpdate, meetings, initialShare, shared, onRename, onDelete, role, shareTok, user }) {
  ...
  const [docData, setDocData] = useState(null); // { doc, meta } when the AI document modal is open
  ...
  const genDoc = async () => {
    setMenu(null);
    if (docBusy) return;
    setDocBusy(true); toast("Building your document…");
    try {
  ...
  return (
    <div className="flex flex-1 overflow-hidden">
      ...
      {docData && <DocModal doc={docData.doc} meta={docData.meta} onClose={() => setDocData(null)} />}
    </div>
  );
}
