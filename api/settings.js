...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === "GET") {
      const u = await sb(`app_users?id=eq.${uid}&select=auto_join,notetaker_name,recall_calendar_id,recall_calendar_status,sharing_prefs`);
      const row = u[0] || {};
      const prefs = { ...PREF_DEFAULTS, ...((row.sharing_prefs && typeof row.sharing_prefs === "object") ? row.sharing_prefs : {}) };
      return res.status(200).json({ auto_join: row.auto_join !== false, notetaker_name: row.notetaker_name || "OctoMeet AI", recall_connected: !!row.recall_calendar_id, recall_status: row.recall_calendar_status || null, sharing_prefs: prefs });
    }
    if (req.method === "POST") {
