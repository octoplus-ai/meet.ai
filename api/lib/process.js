...
export const ANALYSIS_VERSION = 13; // v13: never surface diarization placeholder labels (Speaker A/B/C) in narrative - omit unidentifiable speakers instead of inventing them
...
export async function analyzeTranscript(text, title, participantNames) {
  const sys =
    ...
    "GLOBAL RULE: refer to every person by their NAME only. NEVER output an email address in ANY field (summary, chapters, highlights, participants, action items, anywhere). If the only identifier you have for someone is an email, use the part before the @ as their name (e.g. 'delfinaamelgarejo@gmail.com' -> 'Delfina') or just say 'a participant' - never print the raw email.\n" +
    `{
  "summary": string - a THOROUGH multi-paragraph executive summary that lets someone who missed the meeting fully understand it WITHOUT watching the recording. Write 3 to 5 FULL paragraphs separated by a blank line (a genuinely complete recap, roughly 250 to 450 words). NEVER compress it into a single paragraph, and never use bullets, arrays, numbering, or headings (those belong to the other fields). Structure it as: FIRST paragraph = the context and purpose, who took part, and the single headline outcome (bottom line up front); MIDDLE paragraphs = one paragraph for EACH major theme or thread discussed, explained in real depth - what was said, the reasoning and any disagreement or tension (the WHY, not just the what), and where it landed; FINAL paragraph = the concrete decisions and commitments (with who now owns each), what stays open, contested, or deferred and why, and the agreed direction going forward. Complement, never duplicate, the topics, chapters, keyQuestions, actionItems, nextSteps, and highlights fields: weave their through-line into the narrative instead of re-listing them. Stay strictly specific and neutral, naming the real people BY THEIR NAME ONLY (NEVER write an email address - not in parentheses, not anywhere), plus the numbers, dates, and decisions actually discussed, with no hype, praise, spin, vague filler, speculation, or invented facts. Write it in the meeting's own auto-detected language. Never use em or en dashes; use a short hyphen or reword. Degrade gracefully: if the transcript is very short, empty, unclear, or off-topic, still return a brief honest best-effort summary of whatever is present rather than padding it,
  ...
  "category": string (classify the meeting into ONE concise folder category, Title Case, 1-3 words. Prefer one of: "Sales Call", "Sales Strategy", "Customer Success", "Customer Support", "Customer Feedback", "Onboarding", "One-on-One", "Planning Meeting", "Partnership Alignment", "Product Demo", "Job Interview", "Program Interview", "Professional Consultation", "Technical Troubleshooting", "Training", "Educational", "Standup", "Kickoff", "Team Meeting"; if none fits, invent a fitting concise category. This is the meeting TYPE, used to auto-file it into a folder.)
    ...
    "SPEAKER NAMES - IMPORTANT: when the transcript uses anonymous diarization labels (Speaker A, Speaker B...), figure out who each one actually IS (see speakerMap above) and use the REAL names - never the raw labels - in EVERY human-readable field: participants, pitchAnalysis.speakers, coaching, keyQuestions, highlights and the summary. A diarization label ('Speaker A/B/C', 'Guest', 'Participant N') is NOT a person and MUST NEVER appear in the summary, chapters, highlights or any narrative text. If a speaker genuinely cannot be identified, DO NOT invent, count, or call them out - never write things like 'a third voice identified as Speaker C', 'an unidentified participant', or 'plus Speaker B'. Simply LEAVE THEM OUT of the narrative and write only about the people you can actually name. Better to describe the meeting as being between the named people than to surface a phantom labelled speaker. (The participants array may still list an unmapped speaker, but narrative prose must not.)\n\n" +
  ...
  // Strip any raw emails to a display name (local part, title-cased) so the model never even SEES an
  // email to echo into the summary; the transcript's real names still win via speakerMap.
  const cleanNames = (participantNames || []).map((n) => { const s = String(n || "").trim(); if (!s) return ""; return s.includes("@") ? s.split("@")[0].replace(/[._-]+/g, " ").replace(/\b\w/g, (c) => c.toUpperCase()) : s; }).filter(Boolean);
  const known = cleanNames.length ? `Known participants: ${cleanNames.join(", ")}.\n\n` : "";
